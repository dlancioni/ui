...
select json_agg(t) from (select count(*) over() as record_count,
(tb_event.field->>'id_system')::int as id_system,
(tb_event.field->>'id_group')::int as id_group,
tb_event.id, (tb_event.field->>'id_target')::int as id_target, 
...
(tb_function_id_function.field->>'')::text as function, 
...
inner join tb_domain tb_target_id_target on (tb_event.field->>'id_target')::text = (tb_target_id_target.field->>'key')::text 
    ...
    and (tb_event_id_event.field->>'domain')::text = 'tb_event' where (tb_event.field->>'id_system')::int = 1 
    and (tb_event.field->>'id_target')::int = 1 and (tb_event.field->>'id_table')::int = 1 order by tb_event.id) t